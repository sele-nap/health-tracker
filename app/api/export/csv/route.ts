...
export async function GET(request: Request) {
  ...
  switch (sheet) {
    case 'symptoms': {
      ...
      const defColumns = definitions.map(
        (d) =>
          d.name.toLowerCase().replace(/\s+/g, '_') +
          (d.unit
            ? `_${d.unit.toLowerCase().replace(/\s+/g, '_')}`
            : '_severity'),
      );
    ...
    default:
      return new Response(
        'Invalid sheet parameter. Use: symptoms, medications, appointments, adherence',
        {
          status: 400,
        },
      );
